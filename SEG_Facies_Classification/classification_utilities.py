...
def make_facies_log_plot(logs, pred_column, facies_colors):
    ...
    cluster=np.repeat(np.expand_dims(logs[pred_column].values,1), 100, 1)
...
def compare_facies_plot(logs, pred_column, facies_colors):
    #make sure logs are sorted by depth
    logs = logs.sort_values(by='Depth')
    cmap_facies = colors.ListedColormap(facies_colors[0:len(facies_colors)], 'indexed')
    ...
    cluster2 = np.repeat(np.expand_dims(logs[pred_column].values,1), 100, 1)
    ...
    im2 = ax[6].imshow(cluster2, interpolation='none', aspect='auto', cmap=cmap_facies,vmin=1,vmax=9)
